...
test('control_inventory round-trips the REAL manifest: total == guides+sensors == 131', () => {
  const root = makeRoot();
  const { bundle } = gen(root);
  const inv = bundle.control_inventory;
  assert.strictEqual(inv.total, 131);
  ...
  assert.strictEqual(axisSum, 131);
  assert.strictEqual(statusSum, 131);
...
test('manifest is valid with the new sensor and the control budget holds at 131', () => {
  ...
  assert.strictEqual(counts.guides + counts.sensors, 131);
  const ids = controlIds(manifest);
  assert.strictEqual(ids.length, 131);
  ...
  assert.strictEqual(decision.blocked, false, 'control budget must not block at 131');
});
