...
test('control_inventory round-trips the REAL manifest: total == guides+sensors == 138', () => {
  const root = makeRoot();
  const { bundle } = gen(root);
  const inv = bundle.control_inventory;
  assert.strictEqual(inv.total, 138);
  assert.strictEqual(inv.total, inv.guides + inv.sensors);
  assert.strictEqual(inv.guides, 43);
  assert.strictEqual(inv.sensors, 95);
  const axisSum = Object.values(inv.by_axis).reduce((a, b) => a + b, 0);
  const statusSum = Object.values(inv.by_status).reduce((a, b) => a + b, 0);
  assert.strictEqual(axisSum, 138);
  assert.strictEqual(statusSum, 138);
...
test('manifest is valid with the new sensor and the control budget holds at 138', () => {
  ...
  assert.strictEqual(counts.guides + counts.sensors, 138);
  const ids = controlIds(manifest);
  assert.strictEqual(ids.length, 138);
  ...
  assert.strictEqual(decision.blocked, false, 'control budget must not block at 138');
});
